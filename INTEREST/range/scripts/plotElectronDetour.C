...
void plotElectronDetour()
{
	...
	for (auto name : parameters.names){
		...
		std::vector<double> vecEnergy, vecDetour, vecRMSE;

		double rho = 0.998;
		double energyVar, rangeVar, rmseRangeVar, penVar, rmsePenVar, emptyVar;
		while (inputFile >> energyVar >> rangeVar >> rmseRangeVar 
						 >> emptyVar >> emptyVar >> penVar >> rmsePenVar){
			vecEnergy.push_back(energyVar*1e-6);
			vecDetour.push_back(penVar/rangeVar);
			vecRMSE.push_back(TMath::Sqrt(
				TMath::Power(rmseRangeVar*penVar/(rangeVar*rangeVar), 2)+
				TMath::Power(rmsePenVar/(rangeVar*rangeVar), 2)
			));
		}
		inputFile.close();
