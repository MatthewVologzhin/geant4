...
RunAction::RunAction(){
	...
	fpAccumulableManager->Register(fRange);
...
void RunAction::Book(){
	...
	//1
	fpAnalysisManager->CreateNtuple("ionRanges", "Ranges of ions at different energies");     
    fpAnalysisManager->CreateNtupleDColumn("energy");
    fpAnalysisManager->CreateNtupleDColumn("range"); 
	//2
	fpAnalysisManager->CreateNtuple("calculationTime", "Calculation time at different energies");     
    fpAnalysisManager->CreateNtupleDColumn("energy");
    fpAnalysisManager->CreateNtupleDColumn("time"); 
    //3
	fpAnalysisManager->CreateNtuple("detourFactor", "Detour factor of ions");     
    fpAnalysisManager->CreateNtupleDColumn("energy");
    fpAnalysisManager->CreateNtupleDColumn("detour"); 
    //4
	fpAnalysisManager->CreateNtuple("stoppingPower", "Stopping power (dE/dx) of ions");     
    fpAnalysisManager->CreateNtupleDColumn("energy");
    fpAnalysisManager->CreateNtupleDColumn("sp"); 
    //5
	fpAnalysisManager->CreateNtuple("check", "Stopping power (dE/dx) of ions");     
    fpAnalysisManager->CreateNtupleDColumn("1");
    fpAnalysisManager->CreateNtupleDColumn("2"); 
...
void RunAction::CreateRootFile(){
	...
	if (fpAnalysisManager->IsActive()){		
		...
		if (IsMaster()){		
			...
			newFileName = path + "-" + ionName + "-" + physName + ".root";
			interFileName = path + "-inter" + ".root";
...
void RunAction::GetParticleINFO(const G4Step* step){
	...
	G4double dl = step->GetStepLength();
	if (process){
	...
	if (not(abs(pdgCode) == 11 or pdgCode == 22) and (dl < 1)){
		++fIonSteps;
		fRange += dl;
	}
...
void RunAction::Output(){
	...
	if (IsMaster() and fVerboseLevel > -1){
		...
		G4String outElectronsNb = 	std::to_string(fOuterElectronsNb.GetValue()/fIonsNb.GetValue());
		constDet = G4RunManager::GetRunManager()->GetUserDetectorConstruction();
		det = dynamic_cast<const DetectorConstruction*>(constDet);
		cylinderLength = G4BestUnit(det->GetCylinderLength()/mm, "Length");
		cylinderRadius = G4BestUnit(det->GetCylinderRadius()/mm, "Length");
		LETvalue	   = (fDepositedEnergy.GetValue()/keV)/(det->GetCylinderLength()/um)/fIonsNb.GetValue();
		G4String range = G4BestUnit(fRange.GetValue()/fIonsNb.GetValue(), "Length");
		G4cout << fRange.GetValue() << G4endl;
		G4cout << fIonsNb.GetValue() << G4endl;
		G4cout << fRange.GetValue()/fIonsNb.GetValue() << G4endl;
		G4cout << "---------========= Output of the Run =========---------" << "\n"
			   ...
			   << "Mean fleet out e-:\t"<< outElectronsNb					<< "\n"
