...
void plotProton()
{
	...
	// FIXED BINNING: Центрируем бины на целых числах (от -0.5 до 14.5)
	double xAxisMin = -0.5;
	double xAxisMax = 14.5; 
	unsigned nbBins = 15; 

	// Фиксированные координаты легенды NDC
	double xMinLeg = 0.7; 
	double xMaxLeg = 0.9;
	double yAxisResMin = -3.2;
	double yAxisResMax = 1.2;
	double authorsTextSize = 0.0252;
	double totalNExp = 5e4;
...
const std::string FormCanvasName(std::string path){
	if (path.find("proton") != std::string::npos) return "Ionisation cluster size distribution of proton at energy E=20 MeV";
