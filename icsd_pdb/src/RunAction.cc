...
void RunAction::BeginOfRunAction(const G4Run* run)
{
  ...
  if (!G4Threading::IsMasterThread()) 
  {
      ...
      const auto* pDet = dynamic_cast<const DetectorConstruction*>(pRunManager->GetUserDetectorConstruction());

      if (pMyGen && pDet) {
          const G4ParticleGun* pGun = pMyGen->GetParticleGun();
          G4String detName = pDet->GetDetectorName();
          if (pGun) {
              ...
              if (fFinalFileName == "") {
                  ...
                  fFinalFileName = folder_name + "/ICSD_" + partName + "_" + enName  + "_" + physName + "_" + detName;
              }
          }
      }
  }
