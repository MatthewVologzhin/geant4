...
DetectorConstruction::DetectorConstruction(){
	...
	fWorldSizeY = 40*CLHEP::cm;
	fWorldSizeZ = 40*CLHEP::cm;
...
G4VPhysicalVolume* DetectorConstruction::Construct(){
	...
	G4VPhysicalVolume* pPhysTubs = new G4PVPlacement(pRotation, G4ThreeVector(150.0*CLHEP::cm, 0, 0), pLogicTubs, "physical tube", pLogicWorld, false, 0);
	...
	for (size_t j=1; j < numDetectorsZ; j++){
		...
		G4String fName = std::to_string(i) + "_" + std::to_string(j);
		new G4PVPlacement(0, position, pLogicDetector, fName, pLogicWorld, false, 0);
	}}
