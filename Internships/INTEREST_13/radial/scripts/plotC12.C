...
void plotC12()
{
	...
	/* Experimental data: C12 (24.022 MeV) */
	double LETExp = 440;
	const int n = 7;
	...
	parameters.paths["DNA2"]  = "root/RDD_C12_24.022-MeV_DNA2.root";
	parameters.paths["DNA4"]  = "root/RDD_C12_24.022-MeV_DNA4.root";
	parameters.paths["DNA6"]  = "root/RDD_C12_24.022-MeV_DNA6.root";
	parameters.paths["DNA8"]  = "root/RDD_C12_24.022-MeV_DNA8.root";
	parameters.paths["S4"]  = "root/RDD_C12_24.022-MeV_S4.root";
	...
	for (auto name : parameters.names){
		...
		if (counter == 0){
			...
			pad1->Update(); 
			yAxis->ChangeLabelByValue(1., -1, 0, -1, -1, -1,"");
			/*for (int i=-20; i<20; i++){
	...
	//legend->AddEntry(graphExp, Form("#scale[0.9]{#splitline{%s}{#bf{LET} = %.2f keV/um}}",
...
const std::string FormCanvasName(std::string path){
	if (path.find("C12") != std::string::npos) return "Radial dose distribution of C12 at energy E=2 MeV";
