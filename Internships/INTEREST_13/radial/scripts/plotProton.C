...
void plotProton()
{
	...
	double xData[n] = {0.888529886914378, 1.8578352180936977, 2.1001615508885276, 
					     ...
					     6.865912762520194, 7.67366720516963, 9.289176090468498, 
					     10.50080775444265, 11.87399030694669, 13.974151857835217, 
    ...
    double yData[n] = {149527.54039751206, 38339.206726007076, 15210.94282220963,
						 ...
						 1052.7005084892514, 1759.3537047359928, 753.9167982614002, 
						 340.093642057334, 462.8364025196084, 203.49333335529653, 
	...
	parameters.paths["DNA2"]  = "root/RDD_proton_1-MeV_DNA2.root";
	parameters.paths["DNA4"]  = "root/RDD_proton_1-MeV_DNA4.root";
	parameters.paths["DNA6"]  = "root/RDD_proton_1-MeV_DNA6.root";
	parameters.paths["DNA8"]  = "root/RDD_proton_1-MeV_DNA8.root";
	...
	for (auto name : parameters.names){
		...
		TGaxis *topAxis = new TGaxis(xAxisMin, yAxisMax, xAxisMax, yAxisMax, xAxisMin, xAxisMax, 510, "S-");
		...
		for (size_t i = 0; i < n; i++) {
			...
			double relExpErr = (xExp < 5.0) ? 0.40 : 0.20;
		...
		if (counter == 0){
			...
			errCorridor->SetPointError(0, 0.5, 0.4); 
			for (int i = 1; i < n+1; i++) {
				double x = xData[i];
				double y = yData[i];
				double relErr = (x < 5.0) ? 0.40 : 0.20; // Wingate & Baum total error
			...
			TGaxis *topAxisRes = new TGaxis(xAxisMin, yAxisResMax, xAxisMax, yAxisResMax, xAxisMin, xAxisMax, 510, "-S");
	...
	for (int i=0; i < n; i++){
		double x = xData[i];
		double y = yData[i];
		double totalRelErr = (x < 5.0) ? 0.40 : 0.20;
...
const std::string FormCanvasName(std::string path){
	if (path.find("proton") != std::string::npos) return "Radial dose distribution of proton at energy E=1 MeV";
