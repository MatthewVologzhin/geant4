...
:fDetector(det), fPhysList(pl)
{
  ...
  fCylinderNumberCmd = new G4UIcmdWithAnInteger("/radial/setCylinderNumber",this);
  fCylinderNumberCmd->SetGuidance("Set layers number of cylinder");
  fCylinderNumberCmd->SetParameterName("nbLayers",false);
  fCylinderNumberCmd->SetRange("nbLayers>0");
  fCylinderNumberCmd->AvailableForStates(G4State_PreInit);
  ...
  fCylinderLogCmd = new G4UIcmdWithABool("/radial/setLog", this);
  fCylinderLogCmd->SetGuidance("Set LogX axis of radius");
  fCylinderLogCmd->SetDefaultValue(false);
  fCylinderLogCmd->AvailableForStates(G4State_PreInit);
  fCylinderLogCmd->SetToBeBroadcasted(false);
}
...
DetectorMessenger::~DetectorMessenger()
{
  ...
  delete fCylinderNumberCmd;
  delete fCylinderMinRadiusCmd;
  delete fCylinderLogCmd;
}
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (command == fCylinderNumberCmd)
    fDetector->SetCylinderNumber(fCylinderNumberCmd->GetNewIntValue(newValue));

  if (command == fCylinderMinRadiusCmd)
    fDetector->SetCylinderMinRadius(fCylinderMinRadiusCmd->GetNewDoubleValue(newValue));

  if (command == fCylinderLogCmd)
    fDetector->SetCylinderLog(fCylinderLogCmd->GetNewBoolValue(newValue));
}
