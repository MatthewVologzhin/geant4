...
void plotC12Range()
{
	...
	double yAxisMin = 8e-6;   
	double yAxisMax = 1e-0;
	double xAxisMin = 1e-3;
	double xAxisMax = 1e5;
	double nbBins = 1;
	...
	TString prettyName = Form("Range of {}^{12}C");
	TCanvas *mainCanvas = new TCanvas("mainCanvas", prettyName.Data(), 1920, 1080);	
	...
	for (auto name : parameters.names){
		...
		if (counter == 0){
			pad1->cd();
			TH1F* hFrame1 = new TH1F("hFrame1", prettyName.Data(), nbBins, xAxisMin, xAxisMax);
			hFrame1->SetStats(0);
			TAxis* xAxis = hFrame1->GetXaxis();
			TAxis* yAxis = hFrame1->GetYaxis();
			hFrame1->Draw("AXIS"); // Рисуем рамку первой
			xAxis->SetLimits(xAxisMin, xData[n-1]);
			xAxis->SetRangeUser(xAxisMin, xData[n-1]);
			...
			TLatex *texTitle = new TLatex(0.5, 0.955, prettyName.Data());
			texTitle->SetNDC();
		...
		TGaxis *topAxis = new TGaxis(xAxisMin, yAxisMax, xData[n-1], yAxisMax, xAxisMin, xData[n-1], 510, "GS-");
		...
		if (counter == 0){
			...
			hFrame->SetStats(0);
			hFrame->Draw("AXIS");
			XaxisRes->SetLimits(xAxisMin, xData[n-1]);
			XaxisRes->SetRangeUser(xAxisMin, xData[n-1]);
			...
			TGaxis *topAxisRes = new TGaxis(xAxisMin, yAxisResMax, xData[n-1], yAxisResMax,
									xAxisMin, xData[n-1],
									numDecadesForTGaxis,
									chopt.Data());
