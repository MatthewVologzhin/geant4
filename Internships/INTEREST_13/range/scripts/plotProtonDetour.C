...
void plotProtonDetour()
{
	...
	for (auto name : parameters.names){
		...
		if (counter == 0){
			...
			yAxis->SetTitle("Detour factor   #it{D} =  #frac{#scale[0.8]{Range}}{#scale[0.8]{Penetration}}");
	...
	std::string titleName = "Detour_"+particleName;
