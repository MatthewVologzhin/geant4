...
void C12RadialDose(){
	...
	double LETExp = 440;
	const int n = 7;
	double xData[n] = {0.25413425836515624, 0.8986420254325882, 2.616505975350112,
					   8.728270709892557, 26.420512431971723,95.25836327769836, 357.0609637803312};
    double yData[n] = {6648916.887896762, 918253.4537678715, 147676.72522373096,
					   11787.68683706609, 1095.6773649210595, 68.54554351860529, 1.5695202648183668};
	
	/* Data initialization */
	std::string pathDNA2, pathDNA4, pathDNA6, pathDNA8, pathS4;
	pathDNA2 = "root/simplified-ionradial-C12-24.022_MeV-DNA2.root";	
	pathDNA4 = "root/simplified-ionradial-C12-24.022_MeV-DNA4.root";
	pathDNA6 = "root/simplified-ionradial-C12-24.022_MeV-DNA6.root";
	pathDNA8 = "root/simplified-ionradial-C12-24.022_MeV-DNA8.root";
	pathS4 = "root/simplified-ionradial-C12-24.022_MeV-S4.root";
	...
	yAxisMin = 0.1*yData[n-1];
	yAxisMax = 2*yData[0];
	xAxisMin = xData[0];
	xAxisMax = 12*xData[n-1];
	...
	XaxisRes->ChangeLabelByValue(1e3, -1, -1, -1, -1, -1, " "); // Delete label '100'
	...
	resGraphDNA2->SetLineWidth(lineWidth);
	resGraphDNA2->SetLineColor(colors["DNA2"]);
	resGraphDNA4->SetLineWidth(lineWidth);
	resGraphDNA4->SetLineColor(colors["DNA4"]);
	resGraphDNA6->SetLineWidth(lineWidth);
	resGraphDNA6->SetLineColor(colors["DNA6"]);
	resGraphDNA8->SetLineWidth(lineWidth);
	resGraphDNA8->SetLineColor(colors["DNA8"]);
	resGraphS4->SetLineWidth(lineWidth);
	resGraphS4->SetLineColor(colors["S4"]);
	resGraphDNA2->Draw("L SAME");
	resGraphDNA4->Draw("L SAME");
	resGraphDNA6->Draw("L SAME");
	resGraphDNA8->Draw("L SAME");
	resGraphS4->Draw("L SAME");

	/* The legend: Pad #2 */
	TLegend *legendRes = new TLegend(xMinLegend, 0.3, 0.9, 1);
	legendRes->SetTextSize(0.055);
	legendRes->AddEntry(resGraphDNA2, Form("#bf{DNA Opt2} | #bf{RMSE} = %.4f", rmseDNA2), "l");
	legendRes->AddEntry(resGraphDNA4, Form("#bf{DNA Opt4} | #bf{RMSE} = %.4f", rmseDNA4), "l");
	legendRes->AddEntry(resGraphDNA6, Form("#bf{DNA Opt6} | #bf{RMSE} = %.4f", rmseDNA6), "l");
	legendRes->AddEntry(resGraphDNA8, Form("#bf{DNA Opt8} | #bf{RMSE} = %.4f", rmseDNA8), "l");
	legendRes->AddEntry(resGraphS4, 	Form("#bf{EM Standard Opt4} | #bf{RMSE} = %.4f", rmseS4), "l");
	legendRes->Draw();
